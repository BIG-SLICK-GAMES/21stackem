...
interface HubSessionContextValue {
  consumeHubToken: (token: string) => Promise<HubProfile>;
...
export function HubSessionProvider({
  ...
  async function consumeHubToken(nextToken: string) {
    const normalizedToken = nextToken.trim();

    if (!normalizedToken) {
      throw new HubApiError("A valid shared auth token is required.", 401);
    }

    return applyAuthenticatedSession(normalizedToken);
  }
