...
Copy `.env.example` to `.env` and point the frontend at the backend you are actually running.

For the same startup model as 21 Holdem on this machine, start the Holdem Game backend first:
...
This uses the same 21 Holdem backend process on port `3050`, exposed on the LAN address instead of a machine-only host.

If you later deploy to staging or production, replace those values with the real API and websocket origins.
...
5. Keep the API behind a real HTTPS origin such as `https://api.bigslickgames.com`.
